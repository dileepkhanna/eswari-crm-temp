...
def get_accessible_user_ids(user):
    ...
    if user.role == 'admin':
        ...
        # Manager can access their own data and their employees' data
        employee_ids = list(
...
def filter_by_user_access(queryset, user, assigned_to_field='assigned_to', created_by_field='created_by'):
    """
    Filter a queryset based on user's role and accessible users.
    
    This function filters data so that:
    - Admins see all data
    - Managers see their own data + their employees' data
