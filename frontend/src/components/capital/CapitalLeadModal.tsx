...
export default function CapitalLeadModal({ lead, employees, currentUserId, currentUserRole, onClose, onSave }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-background rounded-2xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div><label className="text-sm font-medium">Notes</label>
            <textarea rows={2} className={inp} value={form.description} onChange={e => setForm({...form, description: e.target.value})} /></div>
