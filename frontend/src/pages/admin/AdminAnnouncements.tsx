...
export default function AdminAnnouncements() {
  ...
  // Company selection state (Admin-specific)
  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      const transformedAnnouncements: Announcement[] = announcementsData.map((item: any) => ({
        ...
        companies_detail: item.companies_detail || [],
      }));
  ...
  const loadCompanies = async () => {
    ...
    try {
      ...
      logger.log('🏢 Loaded companies for admin:', response.length);
    } catch (error) {
  ...
  const handleEditClick = (announcement: Announcement) => {
    ...
    if (announcement.companies_detail && announcement.companies_detail.length > 0) {
      setSelectedCompanyIds(announcement.companies_detail.map((c: any) => c.id));
    } else {
      setSelectedCompanyIds([]);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Add company IDs for admin users
      selectedCompanyIds.forEach(companyId => {
        formDataToSend.append('company_ids', companyId.toString());
      });
      
      logger.log('🏢 Admin announcement will be sent to companies:', selectedCompanyIds);
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className="font-semibold hidden lg:table-cell">Companies</TableHead>
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto space-y-4 py-4 pr-2">
                ...
                {/* Company Selection (Admin-specific) */}
                <div className="space-y-2">
